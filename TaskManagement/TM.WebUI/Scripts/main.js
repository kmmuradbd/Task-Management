    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function (index, value) {
                /*$('#notiContent').append($('<li>New Task : ' + value.Name + ' (' + value.Status + ') added</li>'));*/
                $('#notiContent').append(
                    $('<li>').append(
                        $('<a>')
                            .attr('href', '/MemberTask')
                            .attr('title', 'View Task: ' + value.Name)
                            .css({
                                'font-weight': 'bold',   // bold text
                                'font-size': '16px',     // larger font
                                'color': '#fff'          // optional: black color
                            })
                            .text('New Task: ' + value.Name + ' (' + value.Status + ') added')
                    )
                );
